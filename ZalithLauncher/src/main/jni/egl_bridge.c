...
// -------------------------------------------------------------
// System EGL/GLES Pointers
// -------------------------------------------------------------
static void* lib_egl_handle = NULL;
static void* lib_gles_handle = NULL; // [NEW] 用于获取 GLES 函数
...
static void* (*sys_eglGetProcAddress)(const char*) = NULL;
...
static int load_system_egl_functions() {
    printf("EGLBridge: Loading System EGL/GLES libraries...\n");
    ...
    // [NEW] Load GLESv2 (for glGetString etc)
    lib_gles_handle = dlopen("libGLESv2.so", RTLD_LAZY);
    if (!lib_gles_handle) lib_gles_handle = dlopen("/system/lib64/libGLESv2.so", RTLD_LAZY);
    if (!lib_gles_handle) lib_gles_handle = dlopen("/system/lib/libGLESv2.so", RTLD_LAZY);
    
    if (!lib_egl_handle || !lib_gles_handle) {
        printf("EGLBridge: [FATAL] Could not dlopen libEGL.so or libGLESv2.so!\n");
        return 0;
    }
    ...
    sys_eglGetProcAddress = dlsym(lib_egl_handle, "eglGetProcAddress");
    ...
    printf("EGLBridge: System Libraries Loaded.\n");
    return 1;
}
...
EXTERNAL_API void* pojavCreateContext(void* contextSrc) {
    ...
    const EGLint ctx_attribs[] = { 0x3098, 3, EGL_NONE };
...
// -------------------------------------------------------------
// 4. [CRITICAL FIX] GetProcAddress Implementation
// -------------------------------------------------------------
// 这里的名字必须是 pojavGetProcAddress，因为 liblwjgl_glfw.so 会找这个名字
EXTERNAL_API void* pojavGetProcAddress(const char* procname) {
    if (!procname) return NULL;

    // 1. 尝试从 GLESv2 系统库加载 (核心函数: glGetString, glDrawArrays 等)
    // Android 的 eglGetProcAddress 经常拿不到核心函数，所以必须 dlsym
    void* addr = dlsym(lib_gles_handle, procname);
    
    // 2. 如果找不到，尝试从 EGL 系统库加载
    if (!addr) {
        addr = dlsym(lib_egl_handle, procname);
    }
    
    // 3. 如果还找不到，尝试使用 eglGetProcAddress (扩展函数)
    if (!addr && sys_eglGetProcAddress) {
        addr = sys_eglGetProcAddress(procname);
    }
    
    // Debug 打印 (可选，如果日志太多可以注释掉)
    // if (!addr) printf("EGLBridge: Failed to load symbol: %s\n", procname);
...
EXTERNAL_API void* pojavGetCurrentContext() { return (void*)g_Context; }
